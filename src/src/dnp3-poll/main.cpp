...
int main(int argc, char* argv[])
{
    ...
    app.add_option("-l,--local", localAddr, "The local DNP3 link address.")->required();
    app.add_option("-r,--remote", remoteAddr, "The remote DNP3 link address.")->required();
    ...
    // 127.0.0.1 20000
    auto channel = manager.AddTCPClient("tcpclient", logging, ChannelRetry::Default(),
                                        {IPEndpoint(ip, port)}, "0.0.0.0", PrintingChannelListener::Create());
